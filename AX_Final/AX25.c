...
#define MY_ADD   CubeSat_Add
#define Dest_ADD  GCS_Add
...
u8 V_S,V_A,V_R;
u8 N_S,N_R;
extern uint8_t SSP_L;
AXDL_State_t AXDL_State;  //DataLink Current state  of the State machine
...
DL_Errorst DL_Error;			
u8 Q_Send_Counter;
u8 Q_Received_Counter;

void AX_InitVariables (void){
	Q_Send_Counter=0;
	Q_Received_Counter=0;
	V_S=0;
	V_A=0;
	V_R=0;    
	N_S=0;
	...
	//T107;
	//	T107_Epired=0;
	DLtoDLS_Primitives=DL_None;
	DLStoDL_Primitives=DL_None;
	LMtoDL_Primitives=LM_None;
	DLtoLM_Primitives=LM_None;
	for(int i=0;i<=8;i++){
		I_Q_Send[i].Empty=1;
	}
	//	LMtoPH_Primitives=PH_None;
	//	PHtoLM_Primitives=PH_None;
	//	PHtoRF_Primitives=RF_None;
...
//    {
//       case LM_EXPEDITED_DATA_Request : /* set LM TO PH Primitive*/
//																				LMtoPH_Primitives=PH_EXPEDITED_DATARequest;
//                                        break;
...
//    	                                }
//    	                                */
//																			LMtoPH_Primitives=PH_SEIZERequest;
//																			//change state
...
//                                        global_u8CurrentDL = global_u8RequestingDL;
//                                        /* move all event submitted by current data link to current queue*/
//                                        for(u8 c=0;c<global_u8NumberOfPrimitive;c++)
//                                        {
//                                            dequeue(&awaiting_queue,&local_u8CopyPrimitive,&local_u8copyRequestingNo);
//           	                                enqueue(&current_queue,local_u8CopyPrimitive,local_u8copyRequestingNo);
//                                        }
...
u8 RC=0;
u8 DataLink_Id=0;
extern uint8_t SSP_Arr_Rx[300];
extern  uint8_t SSP_L;
extern uint8_t SSP_Arr[300];
extern uint8_t FrameASCII[300];
extern uint8_t DLS_CR;
void AX_DLInit(void) {
...
void AX_AwaitingConnectionState(void) {
	switch (LMtoDL_Primitives) {
	case LM_DATAIndicate : AX_FrameProcess(); AX_AwaitingConnectionHandler(); LMtoDL_Primitives=LM_None;goto end;break ;
...
void AX_AwaitingReleaseState(void) {
	switch (LMtoDL_Primitives) {
	case LM_DATAIndicate : AX_FrameProcess(); AX_AwaitingReleaseHandler(); LMtoDL_Primitives=LM_None;
	if(DLtoDLS_Primitives==DL_DISCONNECTConfirm) goto end ;break ;
...
void AX_ConnectedState(void) {
	...
	default 						 : AXDL_State=Connected ;LMtoDL_Primitives=LM_None;break;
	}
	switch (DLStoDL_Primitives) {
	...
	case DL_DISCONNECTRequest:		//Discard Frame Queue   Setting Empty Flag to 1 ?
		...
		AXDL_State=Awaiting_Release;
		DLStoDL_Primitives=DL_None;break;
	case DL_DATARequest			:  //Push on I Frames Queue  I_Send_Queue
		/*AX_SendConfig.NS=0;
																	if(MY_ADD==GCS_Add)
																	{AX_SendConfig.SSID_C=Command;}
																	else
																	...
																	AX_BuildFrame(&AX_SendConfig);*/
		I_Q_Send[Q_Send_Counter].Empty=0;
		cpyArr(&I_Q_Send[Q_Send_Counter].Data,&SSP_Arr_Rx,8);
		I_Q_Send[Q_Send_Counter].Data_Length=8;
		DLtoDLS_Primitives=DL_DATAIConfirm;
		AXDL_State=Connected;
		DLStoDL_Primitives=DL_None;break;
	}
	while(I_Q_Send[Q_Send_Counter].Empty==0){
		if(DL_Flags.PRB==0){
			if(V_S!=V_A+k){
				...
				AX_SendConfig.Frame_Type=I_Frame;
				AX_SendConfig.SSID_C=Command;
				AX_SendConfig.NR=N_R;
				AX_SendConfig.NS=N_S;
				AX_BuildFrame(&AX_SendConfig);
				V_S++;
				Q_Send_Counter++;
				if(Q_Send_Counter==7){
				...
				if(V_A==V_S){
					...
					if(DL_Flags.ORB==1){
						...
						if(AX_ReceiveConfig.NR==V_S){
							...
							while(I_Q_Receive[V_R].Empty==1){ // 1 ?
								if(I_Q_Receive[V_R].Seq==V_R){
									//Copy Array Prefered
									cpyArr((&I_Q_Receive[V_R].Data),(&AX_Received.AX_Data),(AX_Received.AX_Length));
									//*I_Q_Receive[V_R].Data=*AX_Send.AX_Data;
									I_Q_Receive[V_R].Data_Length=AX_Received.AX_Length;
									//I_Q_R Empty =0;
									cpyArr(&SSP_Arr_Rx,(&I_Q_Receive[V_R].Data),(AX_Received.AX_Length));
									// for (count=0 ; count<(8); count++)
									// {
									//	 SSP_Arr_Rx[count]=I_Q_Receive[V_R].Data[count];
									// }
									SSP_L=AX_Send.AX_Length;
									DLtoDLS_Primitives=DL_DATAIndicate;
									V_R++;
								}
							}
...
u16 gen_crc16AX(const u8 *data, u16 size){
	u16 out = 0;
...
void AX_BuildFrame(AX_Config_t *Copy_AXConfig){
	...
	i=0;
	AX_L=17+I_Q_Send[i].Data_Length+2+1;
	//I_Q_Send[i].Empty=1;
	break;
	...
	if(AX_SendConfig.Expedited==0){
		DLtoLM_Primitives=LM_DATARequest;
		for(loc_u8Iterator=0;loc_u8Iterator<=AX_L;loc_u8Iterator++)
					{MUSART_voidTransmitByte(USART1, AX_SendArray[loc_u8Iterator]);}
		//enqueue(&awaiting_queue,DLtoLM_Primitives,DataLink_Id);
	}
	else{
		for(loc_u8Iterator=0;loc_u8Iterator<=AX_L;loc_u8Iterator++)
...
void  AX_FrameProcess(void){
	...
	if(AX_ReceiveArray[0]==AX_FLAG){
		...
		if (cmpArr(&AX_ReceiveArray[1],&MY_ADD,6)){
			...
			u8 x=0;
			x = (GET_BIT(AX_ReceiveArray[15],0));	//Get the first bit of Control
			AX_TempReceivedConfig.PF_Bit=0;
			AX_TempReceivedConfig.PF_Bit|=(0<<GET_3BIT(AX_ReceiveArray[15],4));
			...
			if(x==0){
				...
				if(AX_ReceiveArray[17+AX_TempReceived.AX_Length+2]!=AX_FLAG){
				...
				if(x==1){ //U Frame
					AX_TempReceivedConfig.Frame_Type=U_Frame;
					//U Frame
					/*
				switch (AX_ReceiveArray[15]){
					...
					AX_TempReceivedConfig.Frame_Type=S_Frame;
					//S Frame
					//SS bits in Control
					/*
		...
		if(AX_TempReceivedConfig.Error==Error_None){
			if(AX_TempReceivedConfig.Frame_Type==I_Frame){
				...
				AX_Received.AX_Length=AX_TempReceived.AX_Length;
				cpyArr(&AX_Received.AX_Data,&AX_TempReceived.AX_Data,AX_TempReceived.AX_Length); //USED Length Byte as only the Data size
				cpyArr(&I_Q_Receive[V_R].Data,&AX_TempReceived.AX_Data,AX_TempReceived.AX_Length); //USED Length Byte as only the Data size
...
//  u8 UART_Status=0;
//	UART_Status=USART_u8ReceiveSSP(USART_1);
//	while(UART_Status){
//		SSP_FrameR[SSP_Counter]=((USART1->DR)&0xff);
//		UART_Status=USART_u8ReceiveSSP(USART_1);
//		SSP_Counter++;
//	}
//	//rx array
...
//	if((SSP_FrameR[0]==SSP_Flag)&&(SSP_FrameR[SSP_Counter-1]==SSP_Flag)){
//	DLStoDL_Primitives=	DL_CONNECTRequest;
//	GPIO_voidSetPinValue(GPIO_PORT_A,GPIO_PIN_2,GPIO_PIN_HIGH);
//	}
...
//		cpyArr(&I_Q_Send[0].Data,&SSP_FrameR,SSP_Counter-1);
//		I_Q_Send[0].Data_Length=SSP_Counter-1;
//		//*I_Q_Send[0].Data=*SSP_FrameR;
...
//		//copy arrays or pointers
//			u8 counter =0;
//		while(I_Q_Receive[counter].Empty==0){
//		cpyArr(&SSP_FrameS,&I_Q_Receive[counter].Data,I_Q_Receive[counter].Data_Length);
//		I_Q_Receive[counter].Empty=1;
//	 //	*SSP_FrameR=*I_Q_Receive[0].Data;
//		//Sending to the GUI
//		USART_voidTransmit(USART_1,&SSP_FrameS);
//		counter++;
//		}
